<template>
  <div class="fixed inset-0 bg-gray-900 flex flex-col px-5 justify-center items-center">
    <div class="bg-gray-800 p-6 rounded-lg w-full max-w-md mx-5 md:mx-0">
      ...
      <div v-else>
        <h2 v-if="exercise" class="text-xl text-white font-bold mb-4">
          #{{ exercise?.rank }} {{ exercise?.muscle }} • {{ exercise?.name }}
          <span class="block text-sm text-gray-300">
            {{ exercise?.sets }}x{{ exercise?.reps }}, 
            {{ exercise?.weight }}kg, {{ exercise?.rest }}min rest
          </span>
        </h2>
    ...
    <div v-if="!isLoading" @click="toggleSkip" class="text-white mt-5 py-5 px-14 text-center cursor-pointer">Skip</div>
  </div>
</template>
...
<script setup>
  ...
  watch(exercise, () => {
    if (!exercise?.value?.sets) {
      ...
      setsData.value = Array.from({ length: exercise.value.sets }, () => ({
        reps: { value: '', error: '' },
        weight: { value: '', error: '' },
      }))
    }
  ...
  const add = () => {
    ...
    if (setsData.value.length) {
      ...
      setTimeout(() => {
        const repsString = setsData.value.map(set => set.reps.value || exercise.value.reps).join(':')
        const weightString = setsData.value.map(set => set.weight.value || exercise.value.weight).join(':')

        addStat(exercise.value.documentId, repsString, weightString)
        isLoading.value = false
        router.push(`${rank + 2}`)
      }, 250)
    }
