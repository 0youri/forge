      ...
      <div class="bg-gray-800 p-6 rounded-lg w-full max-w-md mx-5 md:mx-0">
        ...
        <form @submit.prevent="edit">
          ...
          <input 
            type="text"
            placeholder="Type"
            class="w-full p-2 mt-1 border rounded text-black outline outline-offset-1 outline-2 mb-4 outline-yellow-500"
            v-model="type"
          />
          ...
          <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-blue-500">Save</button>
          </div>
        </form>
      </div>
    </div>
...
<script setup>  
  ...
  const edit = () => {
    if( name.value !== '' ){
      editWorkout(props.workout.workoutId, name.value, type.value)
      emit('dismiss')
    } else {
      if ( name.value === '' ){
